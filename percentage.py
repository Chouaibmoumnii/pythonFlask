            ...
            if match_date and match_date < today_date:
                ...
                if team1 and team2:
                    ...
                    if team1_name not in historical_matches:
                        historical_matches[team1_name] = {'goals_scored': 0, 'goals_conceded': 0, 'wins': 0}
                    if team2_name not in historical_matches:
                        historical_matches[team2_name] = {'goals_scored': 0, 'goals_conceded': 0, 'wins': 0}
                    ...
                    # Determine match winner and update wins count
                    if team1_goals[0] > team2_goals[0]:
                        historical_matches[team1_name]['wins'] += 1
                    elif team2_goals[0] > team1_goals[0]:
                        historical_matches[team2_name]['wins'] += 1
            else:
                # Match is upcoming
                team1_id = match['team1']
                team2_id = match['team2']
...
def predict_match_winner(team1_name, team2_name, historical_matches):
    ...
    # Calculate win percentage based on historical wins
    total_wins = team1_stats['wins'] + team2_stats['wins']
    team1_win_percentage = (team1_stats['wins'] / total_wins) * 100 if total_wins > 0 else 0
    team2_win_percentage = (team2_stats['wins'] / total_wins) * 100 if total_wins > 0 else 0
...
def home():
    ...
    for match_id, details in upcoming_matches.items():
        team1_win_percentage, team2_win_percentage = predict_match_winner(details['team1_name'], details['team2_name'], historical_matches)
